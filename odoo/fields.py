...
class Datetime(Field):
    ...
    def convert_to_export(self, value, record):
        if not value:
            return ''
        return self.from_string(Datetime.context_timestamp(record, Datetime.from_string(value)).strftime(DATETIME_FORMAT)) if record._context.get('export_raw_data') else ustr(Datetime.to_string(Datetime.context_timestamp(record, Datetime.from_string(value))))
